...
public class ManagerFlow {
    ...
    private void showMaterials() {
        ...
        for (String code : codes) {
            int qty = store.materials.getOrDefault(code, 0);
            System.out.printf("- %s : %d%n", code, qty);
        }
    }

    private void addMaterial() {
        String code = InputHelper.ask("รหัสวัตถุดิบใหม่ (เช่น WRAP_WHITE, FRESH_ROSE) : ").trim();
        if (code.isEmpty()) {
            System.out.println("ยกเลิก (รหัสว่าง)");
            return;
        }
        String name = InputHelper.ask("ชื่อแสดงผล (เว้นว่างได้) : ").trim();
        int qty = InputHelper.askInt("จำนวนเริ่มต้น : ");
        try {
            store.upsertMaterial(code, name, Math.max(0, qty));
    ...
    private void adjustMaterial() {
        String code = InputHelper.ask("รหัสวัตถุดิบที่ต้องการปรับ : ").trim();
