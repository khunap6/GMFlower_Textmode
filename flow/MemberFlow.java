...
private void customBouquetMenu() {
    ...
    // 4) เลือกการห่อ
    System.out.println("เลือกการห่อกระดาษ");
    System.out.println("[1] กระดาษขาวคลาสสิก");
    System.out.println("[2] กระดาษสีโทนกลาง");
    System.out.println("[3] กระดาษโทนพาสเทล");
    System.out.println("[4] กระดาษหรือโบว์สำหรับ (มีสีให้เลือก)");
    System.out.println("[0] กลับ");
    sel = InputHelper.askInt("กรุณาเลือกเมนูที่ต้องการ : ");
    if (sel == 0) return;
    String[] wraps = { "กระดาษขาวคลาสสิก", "กระดาษสีโทนกลาง", "กระดาษโทนพาสเทล", "กระดาษหรือโบว์สำหรับ (มีสีให้เลือก)" };
    if (sel < 1 || sel > 4) {
        System.out.println("ตัวเลือกไม่ถูกต้อง");
        return;
    }
    ...
    while (true) {
        ...
        if (ch == 1) break;
        switch (ch) {
            case 2 -> { price += 20;  addons.add("การ์ดอวยพร"); }
            case 3 -> { price += 120; addons.add("ตุ๊กตาหมีขนาดเล็ก"); }
            case 4 -> { price += 100; addons.add("ช็อกโกแลตแบบกล่อง"); }
            case 5 -> {
                price += 240;
                if (!addons.contains("การ์ดอวยพร")) addons.add("การ์ดอวยพร");
                if (!addons.contains("ตุ๊กตาหมีขนาดเล็ก")) addons.add("ตุ๊กตาหมีขนาดเล็ก");
                if (!addons.contains("ช็อกโกแลตแบบกล่อง")) addons.add("ช็อกโกแลตแบบกล่อง");
            }
            default -> {}
        }
    }
    ...
    // 6) สรุปชื่อ + ทำรายการ
    String name = "ช่อดอกไม้สั่งจัดแต่งเอง: [" + flowerCategory + "] "
        ...
        if (csel == 1) {
            ...
            while (true) {
                ...
                if (store.hasUserUsedCoupon(code, m.getUsername())) {
                    System.out.println("คูปองนี้ถูกใช้โดยบัญชีของคุณไปแล้ว ใช้ได้ 1 ครั้งต่อผู้ใช้");
                    continue;
                }
    ...
    // ===================== เมนูติดตามสถานะ + ยืนยันรับสินค้า =====================
    // ===================== เมนูติดตามสถานะ + ยืนยันรับสินค้า (ไม่อ้างถึง
    // Order.OrderStatus โดยตรง) =====================
